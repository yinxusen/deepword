...
def train_bert_student(
    ...
    sess_dsqn, model_dsqn, saver_dsqn, train_steps_dsqn = prepare_model(
        create_train_student_dsqn_model, hp, "/device:GPU:0", load_drrn_from)

    # save the very first model to verify the Bert weight has been loaded
    if train_steps_dsqn == 0:
        saver_dsqn.save(
            sess_dsqn, ckpt_drrn_prefix,
            global_step=tf.train.get_or_create_global_step(
                graph=model_dsqn.graph))
    else:
        pass

    sw_path_drrn = pjoin(model_path, "dsqn_summaries", "train")
    sw_dsqn = tf.summary.FileWriter(sw_path_drrn, sess_dsqn.graph)
...
def main(data_path, n_data, model_path, game_path, f_games):
    ...
    cmd_args = CMD(
        ...
        model_creator="Attn2LSTMEncoderDSQN",
